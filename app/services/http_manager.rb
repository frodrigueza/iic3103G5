class HttpManager
	...
	def self.recepcionar_oc(id_oc)
		...
		response = HTTParty.post(url , :body => {:id=>id }.to_json , :headers =>{ 'Content-Type' => 'application/json'})
	...
	def self.emitir_factura(id_oc)
		...
		response = HTTParty.put(url , :body => {:oc => body[:id_oc]}.to_json , :headers =>{ 'Content-Type' => 'application/json'})
	...
	def self.pagar_factura(id_f)
		...
		response = HTTParty.post(url , :body => {:id => body[:id_f]}.to_json , :headers =>{ 'Content-Type' => 'application/json'})
